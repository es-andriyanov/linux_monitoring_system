...
function print_config {
  if [[ "${5}" = "1" ]]
  then
    echo -e "\nColumn 1 background = ""${1}"" $(num_to_color "${1}")"
  else
    echo -e "\nColumn 1 background = default $(num_to_color "${1}")"
  fi

  if [[ "${6}" = "1" ]]
  then
    echo -e "Column 1 font color = ""${2}"" $(num_to_color "${2}")"
  else
    echo -e "Column 1 font color = default $(num_to_color "${2}")"
  fi

  if [[ "${7}" = "1" ]]
  then
    echo -e "Column 2 background = ""${3}"" $(num_to_color "${3}")"
  else
    echo -e "Column 2 background = default $(num_to_color "${3}")"
  fi

  if [[ "${8}" = "1" ]]
  then
    echo -e "Column 2 font color = ""${4}"" $(num_to_color "${4}")"
  else
    echo -e "Column 2 font color = default $(num_to_color "${4}")"
  fi
}
